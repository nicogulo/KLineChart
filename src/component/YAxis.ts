...
export default abstract class YAxisImp extends AxisImp implements YAxis {
  ...
  protected override createRangeImp(): AxisRange {
    ...
    if (realFrom === realTo || realRange < minSpan) {
      ...
      realFrom = minCheck
        ? realFrom
        : maxCheck
        ? realFrom - TICK_COUNT * minSpan
        : realFrom - halfTickCount * minSpan;
      realTo = maxCheck
        ? realTo
        : minCheck
        ? realTo + TICK_COUNT * minSpan
        : realTo + halfTickCount * minSpan;
    }
  ...
  protected override createTicksImp(): AxisTick[] {
    ...
    if (displayRange >= 0) {
      ...
      const first = round(
        Math.ceil(displayFrom / interval) * interval,
        precision
      );
      const last = round(
        Math.floor(displayTo / interval) * interval,
        precision
      );
      let n = 0;
      let f = first;
    ...
    ticks.forEach(({ value }) => {
      ...
      const y = this.convertToPixel(
        this.realValueToValue(this.displayValueToRealValue(+value, { range }), {
          range,
        })
      );
      if (shouldFormatBigNumber) {
